...
import { registerInfluncerService } from '../../service/influncer/influncer.service';
import { loginInfluncer } from '../../redux/slice/influncer.slice';

const InfulencerRegister = () => {
  ...
  const validateForm = () => {
    ...
    else if (!/^\d{10}$/.test(registerForm.phone)) {
    ...
    if (validateForm()) {
      ...
      try {
        ...
        if (resp.data && resp.data.statusCode === 201) {
          ...
          dispatch(loginInfluncer(resp.data.data.newUser));
          setInfluncerToken(resp.data.data.accessToken);
          ...
          setRegisterForm({
            ...
            lastname: '',
            phone: '',
            dob: '2001-01-01',  // Reset to default value after successful submission
            confirmPassword: ''
          });
          navigate('/influncer');
          ...
          {isLoading ? (
            ...
            <form className='flex flex-col justify-center items-center mt-5 md:mt-10' onSubmit={handleSubmit}>
              ...
              <div className='flex flex-col justify-center items-center my-2 w-full'>
                ...
                <Button handlClick={() => navigate('/influncer/login')} text={"Login"} className={'py-2 w-1/3 hover:bg-transparent hover:border border-neutral-500 hover:text-black text-2xl my-2 hover:scale-105 duration-500 transition-transform'} />
              </div>
            </form>
          )}
        </div>
...
export default InfulencerRegister;